...
var diagnosticsMetricsSpecified = [for metric in filter(diagnosticMetricsToEnable, item => item != 'allMetrics'): {
  category: metric
  timeGrain: null
  enabled: true
  retentionPolicy: {
...
var diagnosticsMetrics = contains(diagnosticMetricsToEnable, 'allMetrics') ? [
  {
